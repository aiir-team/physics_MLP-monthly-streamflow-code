#!/usr/bin/env python
# ------------------------------------------------------------------------------------------------------%
# Created by "Thieu Nguyen" at 17:06, 26/03/2020                                                        %
#                                                                                                       %
...
from models.main.hybrid_cnn import GaCnn
from utils.IOUtil import _load_dataset__
from utils.Settings import *
from utils.Settings import ga_cnn_final as param_grid
...
def train_model(item):
	...
	paras_name = "hs_{}-ac_{}--fs_{}-ks_{}-ps_{}--ep_{}-ps_{}-pc_{}-pm_{}".format(item["hidden_sizes"], item["activations"], item["filters_size"],
	                                                item["kernel_size"], item["pool_size"], item["epoch"], item["pop_size"], item["pc"], item["pm"])
	root_hybrid_paras = {
	...
	ga_paras = {"epoch": item["epoch"], "pop_size": item["pop_size"], "pc": item["pc"], "pm": item["pm"]}

	md = GaCnn(root_base_paras=root_base_paras, root_hybrid_paras=root_hybrid_paras, cnn_paras=cnn_paras, ga_paras=ga_paras)
	md._running__()
